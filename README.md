...
> [!WARNING]
> **Изоляция тестовых фикстур (`seed.ts`)**:
> Файл `backend/prisma/seed.ts` содержит демонстрационные фикстуры, предназначенные **исключительно для изолированной локальной разработки**.
> Ни при каких обстоятельствах данный скрипт не должен накатываться на staging- или production-базы данных.

* **Ротация скомпрометированных сид-паролей**:
  Пароли демонстрационных аккаунтов из `seed.ts` зафиксированы в публичной истории git-репозитория и являются **скомпрометированными**.
  Если база данных с этими фикстурами была развернута на любом сетевом/тестовом сервере за пределами локальной машины, пароли учетных записей должны быть немедленно сменены через защищенный административный интерфейс.
* **Регламент окружения Production**:
  - Использование тестовых номеров и паролей строго запрещено.
  - Переменные `JWT_ACCESS_SECRET`, `JWT_REFRESH_SECRET` и `VOTE_SIGNING_KEY` обязательны к заполнению в защищенном хранилище секретов. Сервер блокирует запуск при отсутствии секретов в окружении.
